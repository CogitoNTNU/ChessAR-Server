...
public class SetCameraPosition : MonoBehaviour {
  ...
  void SetCamera() {
    ...
    Vector3 position =
        new Vector3(Convert.ToSingle(rho * Math.Sin(phi) * Math.Cos(theta)),
                    Convert.ToSingle(rho * Math.Cos(phi)),
                    Convert.ToSingle(rho * Math.Sin(phi) * Math.Sin(theta)));
    mainCamera.transform.position = position;
    mainCamera.transform.LookAt(chessboard.transform.transform,
                                new Vector3(0, 1, 0));
    chessboard.transform.transform.position -= rotation_offset;
    ...
    var screenshotName = "Chess_Image_" +
                         System.DateTime.Now.ToString("dd-MM-yyyy-HH-mm-ss") +
                         ".png";
    ScreenCapture.CaptureScreenshot(
        System.IO.Path.Combine(screenshotFolderPath, screenshotName), upscale);
  ...
  void SaveLabels() {
    ...
    using (StreamWriter writer = new StreamWriter(filepath, false)) {
      for (int i = 0; i < worldCorners.Length; i++) {
        ...
        {
          writer.WriteLine(0 + " " + screenPosition.x / Screen.width + " " +
                           screenPosition.y / Screen.height + " " + "0.1" +
                           " " + "0.1"); // class x y width height
